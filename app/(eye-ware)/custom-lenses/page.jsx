...
const page = () => {
  ...
  return (
    <main className="pt-[110px] bg-[#F6F7F5] py-12">
      <SubHeader text="Custom Lenses" />
      <div className="max-w-6xl mx-auto mt-2 flex flex-wrap justify-between">
        {topData.map((item, index) => (
          <div key={index} className="w-full">
            {item.description.map((desc, descIndex) => (
              <>
                <p
                  className="font-[600] text-neutral-500 text-base mb-2"
                  key={descIndex}
                >
                  {desc}
                </p>
